// CHALLENGE 1: REVERSE A STRING
// Return a string in reverse
// ex. reverseString('hello') === 'olleh'
function reverseString(str) {
    toRet = "";
    for (const char of str) {
        toRet = char + toRet
    }
    return toRet
}

// console.log(reverseString("Hello"))

// CHALLENGE 2: VALIDATE A PALINDROME
// Return true if palindrome and false if not
// ex. isPalindrome('racecar') === 'true', isPalindrome('hello') == false
function isPalindrome(str) {
    let reversed = reverseString(str)
    return reversed === str
}

// console.log(isPalindrome("mmomm"))
// console.log(isPalindrome("hello"))

// CHALLENGE 3: REVERSE AN INTEGER
// Return an integer in reverse
// ex. reverseInt(521) === 125
function reverseInt(int) {
    let string = int.toString();
    toRet = "";
    for (const char of string) {
        toRet = char + toRet
    }
    return parseInt(toRet)
}

// console.log(reverseInt(123))
// console.log(reverseInt(451))

// CHALLENGE 4: CAPITALIZE LETTERS
// Return a string with the first letter of every word capitalized
// ex. capitalizeLetters('i love javascript') === 'I Love Javascript'
function capitalizeLetters(str) {
    capitalizeNextLetter = true
    toRet = ""
    for(let i = 0; i < str.length; i++){
        if(capitalizeNextLetter && (/[a-zA-Z]/).test(str.charAt(i))){
            toRet += str.charAt(i).toUpperCase()
            capitalizeNextLetter = false
        }else if(str.charAt(i) === " "){
            capitalizeNextLetter = true
            toRet += str.charAt(i)
        }else{
            toRet += str.charAt(i)
        }
    }
    return toRet
}

// console.log(capitalizeLetters("hello i am amal"))
// console.log(capitalizeLetters("hello    whats up"))



// CHALLENGE 5: MAX CHARACTER
// Return the character that is most common in a string
// ex. maxCharacter('javascript') == 'a'
function maxCharacter(str) {
    tracker = {}
    for(char of str){
        if (char in tracker){
            tracker[char] += 1
        }else{
            tracker[char] = 1
        }
    }
    
    maxKey = null
    maxValue = 0
    for(char in tracker){
        if (tracker[char] > maxValue){
            maxValue = tracker[char]
            maxKey = char
        }
    }
    return maxKey
}

// console.log(maxCharacter("javascript"))
// console.log(maxCharacter("hello"))

// CHALLENGE 6: FIZZBUZZ
// Write a program that prints all the numbers from 1 to 100. For multiples of 3, 
// instead of the number, print "Fizz", for multiples of 5 print "Buzz". For numbers 
// which are multiples of both 3 and 5, print "FizzBuzz".
function fizzBuzz() {
    for (let i = 1; i < 100; i++){
        toPrint = ""
        if (i % 3 === 0 ){
            toPrint += "Fizz"
        }if(i % 5 === 0){
            toPrint += "Buzz"
        }if(toPrint.length > 0){
            // console.log(i)
            console.log(toPrint)
            toPrint = ""
        }
    }
}

fizzBuzz()
// Call Function
// const output = maxCharacter("javascript");

// console.log(output);
